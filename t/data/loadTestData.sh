...
# docker run --rm --link fxdata::fxdata -v /root/fxtrader.cfg:etc/fxtrader fxtrader/scripts 'fx-create-db-schema.pl | fx-db-client.pl'
...
mysqlimport -h fxdata -v --ignore --ignore-lines=1 --local --fields-terminated-by='\t' --lines-terminated-by='\n' -s -u$DBUSER -p$DBPASSWD $DBNAME *_*
cd ..
rm -vfR testdata
