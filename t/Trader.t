...
my $account = Finance::HostedTrader::Factory::Account->new( SUBCLASS => 'UnitTest', system => $trendfollow, startDate=> '2011-10-05' )->create_instance();
my $trader = Finance::HostedTrader::Trader->new( system => $trendfollow, account => $account );
isa_ok($trader, 'Finance::HostedTrader::Trader');

is($trader->getEntryValue('EURUSD', 'long'), 1.3255, 'getEntryValue');
is($trader->getEntryValue('EURUSD', 'short'), 1.3267, 'getExitValue');

is($trader->getExitValue('EURUSD', 'long'), 1.3153, 'getEntryValue');
is($trader->getExitValue('EURUSD', 'short'), 1.3893, 'getExitValue');
